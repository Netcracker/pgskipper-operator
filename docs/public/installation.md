...
| patroni.ignoreSlots.ignoreSlotsPrefix | string                                                                          | no        | "cdc_rs_"                                                           | Specifies prefix for ignore Replications slots.                                                                             |
...
| backupDaemon.backupTimeout | int | no | 600 | Specifies the maximum timeout in seconds for a scheduled full backup worker process. The scheduler applies this value only after at least one previous backup completed and recorded `spent_time` metrics. On the first backup, or when metrics are unavailable, no timeout is enforced. When metrics exist, the effective timeout is the greater of this value and twice the duration of the previous backup. |
