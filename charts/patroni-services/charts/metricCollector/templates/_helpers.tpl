...
{{if .Values.siteManager.install }}
    - alert: Standby cluster is not streaming
      annotations:
      ...
      labels:
        ...
        namespace: {{ .Release.Namespace }}
        service: {{ .Release.Name }}
    - alert: Standby cluster has big lag in bytes
      annotations:
        description: 'Standby cluster has big lag in bytes for {{ .Release.Namespace }}'
        summary: Standby cluster has big lag in bytes
      expr: ma_pg_standby_replication_lag_in_bytes{namespace="{{ .Release.Namespace }}"} > {{ default 1073741824 .Values.metricCollector.prometheusRules.standbyLagInBytesThreshold }}
      for: {{ default "3m" .Values.metricCollector.prometheusRules.alertDelay }}
      labels:
        severity: warning
        namespace: {{ .Release.Namespace }}
        service: {{ .Release.Name }}
    - alert: Standby cluster has big lag in milliseconds
      annotations:
      ...
      expr: ma_pg_standby_replication_lag_in_ms{namespace="{{ .Release.Namespace }}"} > {{ default 600000 .Values.metricCollector.prometheusRules.standbyLagInMsThreshold }}
      for: {{ default "3m" .Values.metricCollector.prometheusRules.alertDelay }}
      labels:
        severity: warning
        namespace: {{ .Release.Namespace }}
